...
const BrewNode = function(port){
	...
	function getLatestBlock() {
        return JSON.stringify({block: chain.getLatestBlock()});
    }
	
	function getChainInfo(){
		let bchain = chain.getChain();
		let totalBlocks = chain.getTotalBlocks();
		
		return JSON.stringify({ blocks: totalBlocks, chain: bchain});
	}
    ...
    const createBlock = (temperature,pressure,volume) => {
		let data = "temp="+temperature+"|pressure="+pressure+"|volume="+volume;
